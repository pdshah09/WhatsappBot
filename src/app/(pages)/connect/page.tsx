...
export default function ConnectPage() {
  ...
  const handleConnect = async (sessionId?: string) => {
    ...
    try {
      ...
      // If restoring a saved session, skip QR and go straight to /session
      router.push(sessionId ? "/session" : "/qr");
    } catch {
    ...
    <div className="min-h-screen bg-[#0a0a0a] text-white flex items-center justify-center p-6">
      <div className="bg-[#111] border border-white/10 rounded-2xl p-8 w-full max-w-sm flex flex-col items-center gap-5">
        ...
        {fetching ? (
          ...
          <div className="w-full flex flex-col gap-2">
            <p className="text-white/30 text-xs uppercase tracking-widest mb-1">Saved sessions</p>
            {sessions.map((s) => (
              <button
                key={s.id}
              ...
              >
                ...
                <span className="ml-auto text-white/20 text-xs">Restore →</span>
              </button>
            ))}
