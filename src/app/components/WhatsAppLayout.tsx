...
// WhatsApp-style layout: left sidebar (session + chats) + right thread (messages + compose)
'use client';
...
export default function WhatsAppLayout({ botState, sessionsVersion, onLogout, onSwitch }: Props) {
  ...
  // ── Load chats when active session changes
  ...
  // ── Sync refetchTrigger with sessionsVersion
  ...
  const sendInThread = async () => {
    ...
    } catch { /* silent — server error shown via toast is fine */ }
    finally { setSending(false); }
  };
                ...
                return (
                  <div key={msg.id} className={`flex ${msg.fromMe ? 'justify-end' : 'justify-start'}`}>
                    <div className={`max-w-[70%] px-3 py-2 rounded-2xl text-sm leading-relaxed ${
                      ...
                      {msg.hasMedia && !msg.body && <span className="text-white/40 italic text-xs">📎 Media</span>}
            ...
            <div className="border-t border-white/8 bg-[#111] px-3 py-2.5 flex items-end gap-2">
              ...
              <div className="flex-1 flex flex-col min-w-0">
                ...
                <textarea value={compose} onChange={e => setCompose(e.target.value)} onKeyDown={handleComposeKey}
                  ...
                  className="w-full bg-transparent text-sm text-white/80 placeholder:text-white/25 resize-none focus:outline-none max-h-32 overflow-y-auto leading-relaxed"
