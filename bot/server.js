...
async function createClient() {
  ...
  client = new Client({
    authStrategy: new OptimizedRemoteAuth({
      store,
      backupSyncIntervalMs: 300000, // 5 min — less aggressive than 1 min
    }),
    qrMaxRetries: 5,
    puppeteer: {
...
function bindEvents() {
  ...
  client.on('ready', async () => {
    ...
    // Always backup on ready — idempotent, ensures MongoDB is always up to date
...
mongoose.connect(mongoUri)
  .then(async () => {
    ...
    await createClient();
    clearBrowserLock();
    client.initialize(); // restores session from MongoDB automatically
